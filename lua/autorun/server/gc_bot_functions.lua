...
hook.Add("PlayerSpawn", "GCBotsSpawn", function(bot)
    ...
    if bot:IsBot() then
        if bot.nav != nil then
            ...
            bot.nav = ents.Create("gc_nextbot") // create the nav
...
hook.Add( "StartCommand", "StartCommandExample", function( ply, cmd )
    ...
    if ply.nav:IsValid() == false then
        ply.nav = ents.Create("gc_nextbot") // create the nav
        ply.nav:SetOwner(ply) // set the owner so it doesnt collide
        ply.nav:Spawn() // spawn the nav into the world
    end
    ...
    if look != nil then -- if there are segments
        if look[2] != nil then -- if there is a second segment
            ...
            if targ.type == 0 then -- is the segment a normal walk segment
                ...
                if targ:Distance(bot:GetPos()) < 10 then
                    if look[3] != nil then
