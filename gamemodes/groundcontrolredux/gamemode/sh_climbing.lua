...
--     so I decided to !enable a feature that looks like shit on clients with ping above 50
-- ]]--
...
-- PLAYER.MAX_XY_VELOCITY_FOR_CLIMBING = 400 -- our horizontal velocity can !exceed this much for us to be able to climb
...
--     return !object:IsPlayer() and !object:IsNPC()
-- end

-- function PLAYER:canClimb()
--     local vel = self:GetVelocity()

--     return !self:Crouching() and self:KeyDown(IN_JUMP) and self:Health() >= PLAYER.MAX_CLIMB_HEALTH and vel.z > 0 and (math.abs(vel.x) + math.abs(vel.x)) < PLAYER.MAX_XY_VELOCITY_FOR_CLIMBING and self:GetMoveType() == MOVETYPE_WALK
-- end
...
--     dest.z = dest.z - 0.5 -- reset the offset we applied to !constantly increase the destination Z position
...
--     return math.Distance(pos.x, pos.y, self.climbDestination.x, self.climbDestination.y) > PLAYER.ABORT_DISTANCE
-- end

-- function PLAYER:attemptClimb(moveData)
--     if !self.climbDestination then
...
--                 if !self:tooFarApartFromClimbDest() then
--                     if !self:reachedClimbDestination() then
